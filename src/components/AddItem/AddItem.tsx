		...
		<SelectTag>
			{categories.map((category) => (
				<option
					key={category}
					value={category}
					{...register("category")}>{`${category}`}</option>
			))}
		</SelectTag>
	);
	...
	const onSubmit = async (input: CreateNewItemInput) => {
		...
		try {
			...
			const data: NewProduct = {
				...
				category:
					input.my_category.trim().length > 0 &&
					input.my_category !== "empty"
						? input.my_category.trim()
						: input.category,
				imageUrl,
			};
